...
% Description: Plots the tangent function based on provided parameters and marks
% minima, maxima, and intercepts

function getTan(amplitude, frequency, phaseShift, verticalShift, endPoint, xValues, trigFunc)
    % Syntax:
    %   getTan(amplitude, frequency, phaseShift, verticalShift, endPoint, xValues, phasePoints, trigFunc)
    %
    % Inputs:
    ...
    %   trigFunc - The type of trigonometric function (e.g., 'tan')
    
    syms x
    
    % Define the tangent function based on the parameters
    if frequency > 0
        y = amplitude * tan(frequency * x + phaseShift) + verticalShift;
