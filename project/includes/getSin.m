...
% Description: Plots sin function based on provided parameters and marks
% minima, maxima, and intercepts

function getSin(amplitude, frequency, phaseShift, verticalShift, numCycles, endPoint, xValues, phasePoints, trigFunc)
    % Syntax:
    %   getSin(amplitude, frequency, phaseShift, verticalShift, numCycles, endPoint, xValues, phasePoints, trigFunc)
    %
    % Inputs:
    ...
    if frequency > 0
        ...
        y = amplitude * sin(frequency * xValues + phaseShift) + verticalShift;
    ...
    elseif verticalShift ~= 0 && numCycles == 1
        ...
        plot(endPoint, verticalShift, 'm*', 'MarkerSize', 10);
    end
    hold off;
end
